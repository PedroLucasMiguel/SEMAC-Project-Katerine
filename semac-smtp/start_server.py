...
if __name__ == '__main__':
    ...
    os.rename('emails-queue.txt', f'Logs/{today.timestamp()}-emails-queue.txt')

    with open('emails-queue.txt', 'w') as new_file:
        new_file.close()

    with open('emails-queue.txt', 'r') as queue:
        while True:
            line = queue.readline()
            ...
            if last_valid_line != line:
                if line == '':
                    ...
                    gmail_smtp_service_provider.send_verification_code(email_and_code[0], email_and_code[1])
